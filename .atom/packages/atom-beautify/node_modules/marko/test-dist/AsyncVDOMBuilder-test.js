...
function getChildNodes(parentNode) {
    ...
    var curChild = parentNode.al_;
    while (curChild) {
        childNodes.push(curChild);
        curChild = curChild.aj_;
    }
...
describe('AsyncVDOMBuilder', function () {
    ...
    it('async', function (done) {
        ...
        out.on('finish', function (result) {
            ...
            expect(tree.al_.ag_).to.equal('div');
            expect(tree.al_.aj_.ag_).to.equal('span');
            expect(tree.al_.aj_.aj_.ag_).to.equal('section');
            done();
        });
    });
    ...
    it('for loop', function (done) {
        ...
        out.once('finish', function (result) {
            ...
            expect(header.ag_).to.equal('h1');
            expect(list.ag_).to.equal('ul');
            expect(getChildNodes(list).length).to.equal(10);
            expect(paragraph.ag_).to.equal('p');
            done();
        });
    ...
    it('staticNode, text, comment', function (done) {
        ...
        out.once('finish', function (result) {
            var tree = result.getOutput();
            var childNodes = getChildNodes(tree);
            expect(childNodes[0].ag_).to.equal('div');
            expect(childNodes[1].ao_).to.equal('Hello <em>World</em>');
            expect(childNodes[2].ao_).to.equal('TODO: make this work');
            done();
        });
    });
