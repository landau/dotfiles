...
function getNodeType(node) {
    return node.nodeType || node.ai_;
}

function getNodeValue(node) {
    return node.ao_ || node.nodeValue;
}

function getFirstChild(node) {
    return node.al_ || node.firstChild;
}

function getNextSibling(node) {
    return node.aj_ || node.nextSibling;
}
...
function vdomToHTML(node, options) {
    ...
    function serializeElHelper(el, indent) {
        var tagName = el.nodeName || el.ag_;

        var elNamespaceURI = el.namespaceURI || el.b__;
        ...
        var attributes = el.attributes || el.aZ_;
        var attributesArray = [];
        var attrName;
        ...
        if (tagName.toUpperCase() === 'TEXTAREA') {
            html += indent + '  VALUE: ' + JSON.stringify(ltrim(el.value || el.aq_)) + '\n';
        } else {
