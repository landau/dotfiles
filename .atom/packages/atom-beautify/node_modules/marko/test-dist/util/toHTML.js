function getNodeType(node) {
    return node.nodeType || node.ac_;
}

function getNodeValue(node) {
    return node.ae_ || node.nodeValue;
}

function getFirstChild(node) {
    return node._a_ || node.firstChild;
}

function getNextSibling(node) {
    return node._c_ || node.nextSibling;
}
...
function toHTML(node) {
    ...
    function serializeElHelper(el, indent) {
        var tagName = el.nodeName || el._b_;

        var elNamespaceURI = el.namespaceURI || el.aS_;
        ...
        var attributes = el.attributes || el.aQ_;
        var attributesArray = [];
        var attrName;
